...
clc
rng(555,'philox')
tipo    = 'CGM';            % Conjugate Gradient Method
cant	= 4;                % Cantidad de funciones test
dimensi = 10;                % Cantidad de dimensiones
lim     = 100;              % Cant. veces que se repite el algoritmo aprox 100
longvect= 1000;             % esta relacionado con max_iter
...
for w=1:4
    ...
    matrix_MSE  = ones(lim,longvect)*nan;
    matrix_RMSE = ones(lim,longvect)*nan;
    matrix_MAE  = ones(lim,longvect)*nan;
    matrix_MSPE = ones(lim,longvect)*nan;
    matrix_MAPE = ones(lim,longvect)*nan;
    matrix_RMSLE= ones(lim,longvect)*nan;
    suma    = 0;            % Acumulador
    tic
    cont    = 0;
    for i=1:lim
        [t,suma,bestpos,bestfun,errores_celda,rxi]=CGM(w,suma,p); % Funcion Conjugate Gradient Descent
        bp(i,:) = bestpos;
        bf(i)   = bestfun;
        iter(i) = t-1;
